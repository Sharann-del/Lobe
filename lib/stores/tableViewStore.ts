...
export const useTableViewStore = create<TableViewState>()((set, get) => ({
  ...
  fetchProperties: async (pageIds) => {
    ...
    try {
      ...
      const byPage: Record<string, PageProperty[]> = { ...get().propertiesByPage };
      for (const row of (data ?? []) as PageProperty[]) {
  ...
  updatePropertyValue: async (pageId, key, valueType, value) => {
    const prev = get().propertiesByPage[pageId] ?? [];
    const existing = prev.find((p) => p.key === key);

    const optimistic: PageProperty = existing
      ? { ...existing, value }
      : {
          id: crypto.randomUUID(),
          page_id: pageId,
          key,
          value_type: valueType as PageProperty["value_type"],
          value,
          created_at: new Date().toISOString(),
        };

    set((state) => {
      const list = [...(state.propertiesByPage[pageId] ?? [])];
      ...
      return {
        propertiesByPage: { ...state.propertiesByPage, [pageId]: list },
      };
    });
    ...
    try {
      ...
      set((state) => ({
        propertiesByPage: { ...state.propertiesByPage, [pageId]: prev },
        lastSyncError:
