...
import type { SectionSchemaField, NodeRow } from "@/lib/types/nodes";
...
interface SectionState {
  ...
  updateSchema: (schema: SectionSchemaField[]) => Promise<void>;
  addSchemaField: (field: SectionSchemaField) => Promise<void>;
  updateSchemaField: (
    fieldId: string,
    updates: Partial<SectionSchemaField>
  ) => Promise<void>;
...
export const useSectionStore = create<SectionState>()((set, get) => ({
  ...
  fetchSection: async () => {
    ...
    try {
      ...
      set({ schema: node.section_schema ?? [] });
    } catch (e) {
      set({
        lastSyncError:
          e instanceof Error ? e.message : "Failed to fetch section",
      });
    } finally {
      set({ loading: false });
    }
  ...
  fetchArticles: async () => {
    ...
    try {
      const { data, error } = await supabase
        .from("pages")
        .select("*")
        .eq("parent_id", sectionNodeId)
        .eq("is_deleted", false)
        .order("sort_order", { ascending: true });

      if (error) throw error;
      set({ articles: (data ?? []) as NodeRow[] });
    } catch (e) {
      set({
        lastSyncError:
          e instanceof Error ? e.message : "Failed to fetch articles",
      });
    }
  },
  ...
  updateSchema: async (schema) => {
    ...
    try {
      const { error } = await supabase
        .from("pages")
        .update({ section_schema: schema as unknown as object })
        .eq("id", sectionNodeId);
      if (error) throw error;
    } catch (e) {
      set({
  ...
  createArticle: async (userId, templateId) => {
    const { sectionNodeId, workspaceId, templates, articles } = get();
    if (!sectionNodeId || !workspaceId) return null;

    const template = templateId
      ? templates.find((t) => t.id === templateId)
      : undefined;

    const maxOrder = articles.reduce(
    ...
    try {
      ...
      if (error) throw error;
      const article = data as NodeRow;
      set((s) => ({ articles: [...s.articles, article] }));
      return article.id;
    } catch (e) {
      set({
        lastSyncError:
          e instanceof Error ? e.message : "Failed to create article",
      });
      return null;
    }
  },
  ...
  exportCSV: () => {
    const { schema, articles } = get();
    const headers = ["Title", ...schema.map((f) => f.name)];
    const rows = articles.map((article) => {
      const values = [article.title];
  ...
  exportJSON: () => {
    const { schema, articles } = get();
    return JSON.stringify({ schema, articles }, null, 2);
  },
