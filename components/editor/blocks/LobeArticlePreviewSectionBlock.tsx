...
function LobeArticlePreviewSectionView(props: {
  editor: {
    isEditable: boolean;
    updateBlock: (
      _block: { id: string },
  ...
  return (
    ...
    >
      {pageIds.map((id) => {
        const node = runtime.pages.find((x) => x.id === id);
        const title = node?.title ?? "Article";
        const icon = node?.icon ?? "📄";
        const description =
          node && "description" in node && typeof node.description === "string"
            ? node.description
            : "Open this article to read more.";
        return (
          <div
            key={id}
            className={cn(
