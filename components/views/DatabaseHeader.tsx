...
const VIEW_TABS: ViewTab[] = [
  ...
  { type: "card", label: "Gallery", icon: LayoutGrid },
  { type: "kanban", label: "Board", icon: KanbanSquare },
...
export function DatabaseHeader({
  ...
  onNewEntry,
  onOpenProperties,
}: DatabaseHeaderProps): JSX.Element {
  const [isSearchOpen, setIsSearchOpen] = useState(false);
  const [isEditingTitle, setIsEditingTitle] = useState(false);
  const titleRef = useRef<HTMLInputElement>(null);

  const activeView = useDatabaseStore((s) => s.activeView);
  const setActiveView = useDatabaseStore((s) => s.setActiveView);
  const searchQuery = useDatabaseStore((s) => s.searchQuery);
  const setSearchQuery = useDatabaseStore((s) => s.setSearchQuery);
  const filters = useDatabaseStore((s) => s.filters);
  const sorts = useDatabaseStore((s) => s.sorts);
  const groupByPropertyId = useDatabaseStore((s) => s.groupByPropertyId);
  const schema = useDatabaseStore((s) => s.schema);
  const hiddenPropertyIds = useDatabaseStore((s) => s.hiddenPropertyIds);
  const togglePropertyVisibility = useDatabaseStore(
    (s) => s.togglePropertyVisibility
  );
  const setGroupBy = useDatabaseStore((s) => s.setGroupBy);
  const exportCSV = useDatabaseStore((s) => s.exportCSV);
  const exportJSON = useDatabaseStore((s) => s.exportJSON);
    ...
    <div className={cn("flex flex-col gap-0", className)}>
      ...
      <div className="flex items-center justify-between gap-2 px-6 border-b border-border-subtle">
          ...
          {VIEW_TABS.map((tab) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <motion.div
                    layoutId="database-view-tab-indicator"
        ...
        <div className="flex items-center gap-1 py-1.5">
          ...
          {/* New entry */}
          <Button size="sm" onClick={onNewEntry} className="ml-1">
            <Plus size={14} />
            New
          </Button>
        </div>
