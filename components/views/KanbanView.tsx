...
import { useKanbanViewStore } from "@/lib/stores/kanbanViewStore";
...
export function KanbanView({
  ...
  const groupByPropertyId = useKanbanViewStore((s) => s.groupByPropertyId);
  const subGroupByPropertyId = useKanbanViewStore(
    (s) => s.subGroupByPropertyId
  );
  const collapsedColumns = useKanbanViewStore((s) => s.collapsedColumns);
  const columnOrder = useKanbanViewStore((s) => s.columnOrder);
  const hideEmptyGroups = useKanbanViewStore((s) => s.hideEmptyGroups);
  const cardDisplayFields = useKanbanViewStore((s) => s.cardDisplayFields);
  const setGroupByPropertyId = useKanbanViewStore(
    (s) => s.setGroupByPropertyId
  );
  const setSubGroupByPropertyId = useKanbanViewStore(
    (s) => s.setSubGroupByPropertyId
  );
  const toggleColumnCollapsed = useKanbanViewStore(
    (s) => s.toggleColumnCollapsed
  );
  const setColumnOrder = useKanbanViewStore((s) => s.setColumnOrder);
  const reorderColumns = useKanbanViewStore((s) => s.reorderColumns);
  const setHideEmptyGroups = useKanbanViewStore((s) => s.setHideEmptyGroups);
  const toggleCardDisplayField = useKanbanViewStore(
    (s) => s.toggleCardDisplayField
  );
  const initCardDisplayFields = useKanbanViewStore(
  ...
  const { columnKeys, columnMap, columnOptions } = useMemo(() => {
    if (!groupSchema) {
      return {
        ...
        columnMap: new Map<string, PageRow[]>([[NO_VALUE_KEY, rows]]),
    ...
    (event: DragEndEvent) => {
      ...
      if (activeData?.type === "card" && groupSchema) {
        ...
        if (groupSchema.type === "multi_select") {
          const props = propertiesByPage[cardId] ?? [];
          const prop = props.find((p) => p.key === groupSchema.name);
  ...
  const handleDuplicate = useCallback(
    (pageId: string) => {
      void duplicatePage(pageId, userId);
    },
  ...
  const activeCard = activeCardId ? pagesById[activeCardId] : null;
...
interface FlatBoardAreaProps {
  ...
  activeCard: PageRow | null | undefined;
...
function SwimlaneBoardArea({
  ...
  const swimlanes = useMemo(() => {
    ...
    return entries;
  }, [rows, propertiesByPage, subGroupSchema, hideEmptyGroups]);
